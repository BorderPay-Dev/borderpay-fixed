...
export function allowedRoute(method, path, query) {
  ...
  if (method === "GET" && /^\/(receive|send)\/sequence-id\/[0-9a-f-]{36}$/i.test(path)) return keys.length === 0;
  if (method === "POST" && (path === "/receive" || path === "/send")) return keys.length === 0;
  if (["GET", "POST", "PUT"].includes(method) && path === "/webhooks") return keys.length === 0;
  if (method === "DELETE" && /^\/webhooks\/[0-9a-f-]{36}$/i.test(path)) return keys.length === 0;
  return false;
}
