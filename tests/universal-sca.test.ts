...
Deno.test('SCA canonical payload is stable and excludes credentials and authorization id', async () => {
  ...
  const canonical = scaCanonicalPayload('bridge_transfer', { password: 'secret-password', pin: '123456', totp: '654321' });
  assert(!canonical.includes('secret-password'), 'password must not enter canonical evidence');
  assert(!canonical.includes('123456'), 'PIN must not enter canonical evidence');
});
