...
Deno.serve(async (req) => {
  ...
  if (action === "admin_list_tickets") {
    ...
    let q = supa.from("support_tickets").select("*").order("last_message_at", { ascending: false }).limit(Math.min(limit * 2, 400));
    ...
    const sorted = (rows || [])
      ...
      .sort((a: any, b: any) => {
        ...
        if (p !== 0) return p;
        const aTs = new Date(String(a?.last_message_at || a?.created_at || 0)).getTime();
        const bTs = new Date(String(b?.last_message_at || b?.created_at || 0)).getTime();
        return bTs - aTs;
      })
      .slice(0, limit);
    return json({ success: true, data: { tickets: sorted } });
  }
  ...
  if (action === "admin_ai_draft") {
    ...
    try {
      ...
      const rawMessage = String(e?.message || "Failed to generate AI draft");
      const lower = rawMessage.toLowerCase();
      const message =
        lower.includes("subscription key") || lower.includes("api endpoint")
          ? "AI draft is temporarily unavailable. Please send a manual reply or handoff to human support."
          : rawMessage;
      await supa.from("support_ticket_events").insert({
        ticket_id: ticketId,
        event_type: "ai_draft_failed",
        actor_user_id: user.id,
        payload: { reason: rawMessage.slice(0, 300) },
      });
