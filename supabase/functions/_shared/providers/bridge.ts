 ...
 */ import { bridgeFetch, type BridgeResponse } from "./bridge-client.ts";
import type {
  PaymentProvider,
...
export class BridgeProviderError extends Error {
  ...
  bridge_error?: string;
  raw_text?: string;
  constructor(message: string, meta?: { status?: number; request_id?: string; bridge_code?: string; bridge_error?: string; raw_text?: string }){
    super(message);
...
const KYC_REDIRECT_URL = Deno.env.get("BORDERPAY_APP_URL") ? `${Deno.env.get("BORDERPAY_APP_URL")}/onboarding/kyc-complete` : "https://app.borderpayafrica.com/onboarding/kyc-complete";
export function liquidationAddressDeveloperFeeFields(chain: string, developerFeePercent?: string) {
  const feePercent = String(developerFeePercent || "").trim();
  if (!feePercent || String(chain).toLowerCase() === "tron") return {};
  return {
...
export class BridgeProvider implements PaymentProvider {
  ...
  async createCustomer(input: CustomerCreateInput): Promise<CustomerCreateResult> {
    ...
    body.metadata = {
      borderpay_user_id: input.borderpay_user_id
    };
    ...
    if (!r.ok) {
      const parsed = r.data && typeof r.data === "object" ? r.data : {};
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge createCustomer failed [${r.status}]`, {
        status: r.status,
  ...
  async deleteCustomer(customerId: string): Promise<{ deleted: boolean; raw: unknown }> {
    ...
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge deleteCustomer failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
  ...
  async createKycLink(input: KycLinkInput): Promise<KycLinkResult> {
    ...
    const idemSource = input.customer_id ?? input.email ?? input.full_name ?? crypto.randomUUID();
    const r = await bridgeFetch({
      method: "POST",
      path: "/v0/kyc_links",
      body,
    ...
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge createKycLink failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
    ...
    if (!url || !id) {
      throw new BridgeProviderError("Bridge createKycLink response missing link id/url", {
        status: r.status,
        request_id: r.request_id,
        raw_text: r.raw_text?.slice(0, 1000)
      });
  ...
  async createVirtualAccount(input: VirtualAccountCreateInput): Promise<VirtualAccountResult> {
    ...
    if (!destinationBridgeWalletId || !destinationRail || !input.destination?.currency) {
      throw new BridgeProviderError("Bridge createVirtualAccount request invalid: destination wallet fields are required", {
    ...
    if (!/^\d+(\.\d+)?$/.test(feePercent) || !Number.isFinite(feePercentNumber) || !zeroFeeAllowed && feePercentNumber <= 0 || feePercentNumber >= 100) {
      throw new BridgeProviderError("Bridge createVirtualAccount request invalid: developer_fee_percent must be at least 0 and less than 100", {
        bridge_code: "invalid_parameters",
    ...
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge createVirtualAccount failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
  ...
  async findCustomerByEmail(email: string): Promise<{ id: string; email: string | null; raw: unknown } | null> {
    ...
    for(let page = 0; page < 20; page += 1){
      const r: BridgeResponse<any> = await bridgeFetch({
        method: "GET",
        path: "/v0/customers",
        query: {
          limit: 100,
      ...
      if (!r.ok) {
        ...
        const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
        const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
        throw new BridgeProviderError(`Bridge listCustomers failed [${r.status}]`, {
          status: r.status,
          request_id: r.request_id,
          bridge_code: bridgeCode,
          bridge_error: bridgeErr,
      ...
      const pagination: any = r.data?.pagination ?? r.data?.data?.pagination ?? {};
      const next: unknown = pagination?.next ?? pagination?.next_cursor ?? pagination?.after ?? (r.data?.has_more && rows.length ? rows[rows.length - 1]?.id : undefined);
      if (!next || !rows.length) break;
      cursor = String(next);
    }
    return matches[0] ?? null;
  }
  /** Fetch canonical customer profile fields from Bridge. */ async getCustomerProfile(customerId: string): Promise<any> {
    const r = await bridgeFetch({
      method: "GET",
      path: `/v0/customers/${encodeURIComponent(customerId)}`
    });
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge getCustomerProfile failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
    ...
    const data = r.data?.data ?? r.data ?? {};
    // Bridge SCA scope is based on legal residence for individuals and legal
    // operating address for businesses. Prefer operating/business addresses
    // before generic/registered fallbacks; never let browser metadata decide.
    const addr = data?.operating_address ?? data?.residential_address ?? data?.business_address ?? data?.address ?? data?.registered_address ?? {};
    const countryRaw = addr?.country ?? data?.country ?? data?.country_code ?? data?.residential_address?.country ?? null;
  ...
  /** List the customer's custodial stablecoin wallets. */ async listWallets(customerId: string): Promise<Array<{ wallet_id: string; currency: string; chain: string; address: string; balance?: string; status?: string }>> {
    const rows = await this.fetchBridgeListPaginated({
    ...
    return (Array.isArray(rows) ? rows : []).map((w: any)=>{
      ...
      const raw = w?.currency || w?.symbol || w?.coin || w?.asset_code || w?.asset?.symbol || w?.token || "";
      ...
      return {
        ...
        status: w?.status != null ? String(w.status).toLowerCase() : undefined
      };
    });
  }
  /** Get per-wallet balance rows from Bridge. */ async getWalletBalances(customerId: string, walletId: string): Promise<Array<{ currency: string; chain?: string; balance: string }>> {
    const r = await bridgeFetch({
      method: "GET",
    ...
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge getWalletBalances failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
    ...
    const rows = Array.isArray(payload?.balances) ? payload.balances : Array.isArray(payload) ? payload : [
      payload
    ];
    return (rows || []).map((b: any)=>({
        currency: String(b?.currency || b?.symbol || ""),
        chain: b?.chain ? String(b.chain) : undefined,
        balance: String(b?.balance ?? b?.available_balance ?? b?.available ?? "0")
      }));
  }
  /** List the customer's USD/EUR/GBP virtual accounts. */ async listVirtualAccounts(customerId: string): Promise<Array<{ virtual_account_id: string; currency: string; rail?: string; status?: string; created_at?: string; developer_fee_percent?: number; account_details: unknown }>> {
    const rows = await this.fetchBridgeListPaginated({
    ...
    return (Array.isArray(rows) ? rows : []).map((v: any)=>({
        ...
        developer_fee_percent: v?.developer_fee_percent != null && Number.isFinite(Number(v.developer_fee_percent)) ? Number(v.developer_fee_percent) : undefined,
        ...
        account_details: {
          ...v && typeof v === "object" ? v : {},
          source_deposit_instructions: v?.source_deposit_instructions && typeof v.source_deposit_instructions === "object" ? v.source_deposit_instructions : null
        }
      }));
  }
  /** Deactivate a VA so it cannot accept new incoming transactions. */ async deactivateVirtualAccount(customerId: string, virtualAccountId: string): Promise<ProviderVirtualAccountSummary> {
    const r = await bridgeFetch({
      method: "POST",
    ...
    if (!r.ok) {
      ...
      throw new BridgeProviderError(`Bridge deactivateVirtualAccount failed [${r.status}]`, {
        ...
        bridge_error: typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error,
        raw_text: r.raw_text?.slice(0, 1000)
      });
    }
  ...
  /** Reactivate a provider VA instead of attempting to create a duplicate. */ async reactivateVirtualAccount(customerId: string, virtualAccountId: string): Promise<ProviderVirtualAccountSummary> {
    const r = await bridgeFetch({
      method: "POST",
    ...
    if (!r.ok) {
      const parsed = r.data && typeof r.data === "object" ? r.data : {};
      throw new BridgeProviderError(`Bridge reactivateVirtualAccount failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: typeof parsed.code === "string" ? parsed.code : undefined,
        bridge_error: typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error,
  ...
  private async fetchBridgeListPaginated<T = any>(params: { path: string; context: string; pageSize?: number; maxPages?: number }): Promise<T[]> {
    ...
    while(page < maxPages){
      ...
      if (!r.ok) {
        ...
        const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
        const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
        throw new BridgeProviderError(`Bridge ${params.context} failed [${r.status}]`, {
          status: r.status,
          request_id: r.request_id,
          bridge_code: bridgeCode,
          bridge_error: bridgeErr,
  ...
  async createWallet(input: WalletCreateInput): Promise<WalletResult> {
    ...
    if (!r.ok) {
      if (r.status === 409 || r.status === 422) {
        ...
        const canonical = wallets.find((wallet)=>wallet.chain.toLowerCase() === input.chain.toLowerCase() && Boolean(wallet.address));
        if (canonical) {
          return {
            provider: this.name,
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.error === "string" ? parsed.error : typeof parsed.message === "string" ? parsed.message : r.error;
      throw new BridgeProviderError(`Bridge createWallet failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
    ...
    if (walletId && !depositAddress) {
      ...
      const wallets = await this.listWallets(input.customer_id);
      const canonical = wallets.find((wallet)=>wallet.wallet_id === walletId || wallet.chain.toLowerCase() === input.chain.toLowerCase());
  ...
  async createTransfer(input: TransferCreateInput): Promise<TransferResult> {
    ...
    if (!r.ok) {
      const parsed = r.data && typeof r.data === "object" ? r.data : {};
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.message === "string" ? parsed.message : typeof parsed.error === "string" ? parsed.error : r.error;
      throw new BridgeProviderError(`Bridge createTransfer failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
  ...
  async createLiquidationAddress(input: LiquidationAddressCreateInput): Promise<LiquidationAddressResult> {
    const body = {
      ...
      // Bridge applies the external USDC/Base payout fee on the saved
      // liquidation route through `custom_developer_fee_percent`. Bridge does
      // not accept a developer fee for same-token USDT/Tron routes.
      ...liquidationAddressDeveloperFeeFields(input.chain, input.developer_fee_percent)
    };
    const r = await bridgeFetch({
      method: "POST",
    ...
    if (!r.ok) {
      ...
      const bridgeCode = typeof parsed.code === "string" ? parsed.code : typeof parsed.error_code === "string" ? String(parsed.error_code) : undefined;
      const bridgeErr = typeof parsed.message === "string" ? parsed.message : typeof parsed.error === "string" ? parsed.error : r.error;
      throw new BridgeProviderError(`Bridge createLiquidationAddress failed [${r.status}]`, {
        status: r.status,
        request_id: r.request_id,
        bridge_code: bridgeCode,
        bridge_error: bridgeErr,
