-- Revenue remains earned only while a signed, completed provider deposit has
-- a payment_processed event and no signed terminal refund/return/cancel event.
...
comment on function public.admin_terminal_settled_revenue_summary() is
  'Terminal successful fees valued using linked wallet settlements; signed refunds/returns/cancels, lifecycle duplicates, failures, and pending payments are excluded.';
