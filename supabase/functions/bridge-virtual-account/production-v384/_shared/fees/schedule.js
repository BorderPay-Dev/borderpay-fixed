 ...
 *       • Direct BorderPay virtual-account on-ramp developer fee for future VAs:
 *         - USD: 3.0%
 *         - EUR: 2.98%
 *         - GBP: 2.98%
...
export function borderPayDirectVaDeveloperFeePercent(currency) {
 ...
 */
export const AFRICAN_PAYOUT_MARKUP_PERCENT_BY_PLAN = {
...
export const AFRICAN_RAIL_MARKUP_PERCENT_BY_ACCOUNT = {
    individual: 2.0,
    business: 2.0,
};
export const AFRICAN_RAIL_MARKUP_DEFAULT_PERCENT = 2.0;
export function africanRailMarkupPercentForAccount(accountType) {
 ...
 */
export function bridgeDeveloperFeePercent(paymentRail, _currency, accountType) {
    const rail = String(paymentRail ?? "").toLowerCase();
    if (rail === "external_account_offramp")
        return BRIDGE_DEVELOPER_FEE_PERCENT.external_account_offramp;
    if (rail === "crypto_to_crypto_route")
        return BRIDGE_DEVELOPER_FEE_PERCENT.crypto_to_crypto_route;
    if (rail === "crypto_to_crypto_payout")
        return BRIDGE_DEVELOPER_FEE_PERCENT.crypto_to_crypto_payout;
    return String(accountType ?? "").toLowerCase() === "business"
 ...
 */
export function africanPayoutMarkupPercent(planKey) {
    const k = String(planKey ?? "");
