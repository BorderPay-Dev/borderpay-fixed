...
async function completePartnerInvoice(
  reference: string,
  transactionId: string,
  verified: Record<string, any>,
  eventId: string,
) {
  const { data, error } = await db.rpc("complete_partner_invoice_flutterwave", {
...
async function reconcilePartnerInvoices(providerReference?: string) {
  if (!FLW_SECRET_KEY) return { configured: false, selected: 0, reconciled: 0, pending: 0, failed: 0, results: [] };
  let query = db.from("partner_invoices")
    ...
    .order("created_at")
    .limit(providerReference ? 1 : 100);
  if (providerReference) query = query.eq("payment_reference", providerReference);
  const { data: invoices, error } = await query;
  if (error) throw error;
  ...
  const results: Array<Record<string, unknown>> = [];
  for (const invoice of invoices ?? []) {
    const reference = clean(invoice.payment_reference);
    try {
      const verification = await flutterwave(`/transactions/verify_by_reference?tx_ref=${encodeURIComponent(reference)}`, { method: "GET" });
      const verified = verification?.data ?? {};
      if (clean(verified.status).toLowerCase() !== "successful" || clean(verified.tx_ref) !== reference) {
        pending++;
        results.push({ invoice_id: invoice.id, provider_reference: reference, status: clean(verified.status).toLowerCase() || "not_successful" });
        continue;
      }
      const transactionId = clean(verified.id);
      if (!transactionId) throw new Error("flutterwave_verified_transaction_id_missing");
      const result = await completePartnerInvoice(reference, transactionId, verified, `flutterwave:reconcile:${transactionId}:charge.completed`);
      reconciled++;
      results.push({ invoice_id: invoice.id, provider_reference: reference, status: "paid", result });
    } catch (cause) {
      failed++;
      results.push({ invoice_id: invoice.id, provider_reference: reference, status: "unresolved", error: (cause instanceof Error ? cause.message : String(cause)).slice(0, 180) });
    }
  }
  return { configured: true, selected: invoices?.length ?? 0, reconciled, pending, failed, results };
}
...
async function handleWebhook(req: Request) {
  ...
  const maintenanceInvoice = txRef.startsWith("bp-maintenance-");
  const partnerInvoice = txRef.startsWith("bp-partner-invoice-");
  if (!transactionId || (!maintenanceInvoice && !partnerInvoice)) {
    return json({ success: true, ignored: true });
  }
  ...
  if (partnerInvoice) {
    const data = await completePartnerInvoice(txRef, transactionId, verified, eventId);
    return json({ success: true, signature_version: signatureVersion, invoice_scope: "partner", data });
  }
  ...
  if (error) throw error;
  return json({ success: true, signature_version: signatureVersion, invoice_scope: "maintenance", data });
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body?.mode === "reconcile") {
      ...
      if (providerReference && !providerReference.startsWith("bp-maintenance-") && !providerReference.startsWith("bp-partner-invoice-")) {
        return json({ success: false, error: "invalid_provider_reference" }, 400);
      }
      if (providerReference.startsWith("bp-partner-invoice-")) {
        return json({ success: true, scope: "partner", data: await reconcilePartnerInvoices(providerReference) });
      }
      if (providerReference.startsWith("bp-maintenance-")) {
        return json({ success: true, scope: "maintenance", data: await reconcileInvoices(providerReference) });
      }
      const [maintenance, partner] = await Promise.all([reconcileInvoices(), reconcilePartnerInvoices()]);
      return json({ success: true, scope: "all", data: { maintenance, partner } });
    }
