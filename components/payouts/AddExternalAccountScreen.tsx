/**
 * AddExternalAccountScreen — add a fiat payout destination.
 *
 * Bridge-backed v1 scope:
 *   • US bank account (USD)
 *   • IBAN account (EUR)
 */
...
export function AddExternalAccountScreen({ onBack, onAdded }: AddExternalAccountScreenProps) {
  ...
  const readCachedCapabilities = (): Array<AccountType> => {
    try {
      ...
      return types.filter((x: any) => x === 'us' || x === 'iban');
    } catch {
      return [];
    }
  };
  ...
  useEffect(() => {
    const loadCapabilities = async (force = false) => {
      ...
      const run = (async () => {
        ...
        try {
          ...
          if (r?.success) {
            ...
            const filtered = types.filter((x: any) => x === 'us' || x === 'iban');
            ...
            if (filtered.length > 0) {
              ...
              setAccountType(filtered[0] as AccountType);
            }
          }
        } catch {
          // keep cached capabilities on transient failures
        } finally {
      ...
      try {
        ...
        if (capabilityLoadInFlightRef.current === run) capabilityLoadInFlightRef.current = null;
      }
    };

    void loadCapabilities();
    const onFocus = () => { void loadCapabilities(); };
    const onVisibility = () => { if (document.visibilityState === 'visible') void loadCapabilities(); };
  ...
  return (
    <div className={`min-h-screen ${tc.bg}`}>
      ...
      <header className="flex items-center gap-3 pl-16 pr-5 sm:pr-6 pb-3" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 0.85rem)' }}>
      ...
      <main className="px-5 sm:px-6 pb-10 max-w-md mx-auto space-y-4">
        {capabilityLoading ? (
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setAccountType('us')} disabled={!canUseUs}
                  className={`py-3 rounded-xl border text-sm font-semibold ${!canUseUs ? `bg-white/[0.02] ${tc.textMuted} border-white/10 opacity-50 cursor-not-allowed` : accountType === 'us' ? 'bg-[#C7FF00] text-black border-[#C7FF00]' : `bg-white/[0.04] ${tc.text} border-white/10`}`}>
                  US bank (USD)
                </button>
                <button type="button" onClick={() => setAccountType('iban')} disabled={!canUseIban}
                  className={`py-3 rounded-xl border text-sm font-semibold ${!canUseIban ? `bg-white/[0.02] ${tc.textMuted} border-white/10 opacity-50 cursor-not-allowed` : accountType === 'iban' ? 'bg-[#C7FF00] text-black border-[#C7FF00]' : `bg-white/[0.04] ${tc.text} border-white/10`}`}>
                  IBAN (EUR)
                </button>
              </div>
              <p className={`text-[11px] ${tc.textMuted} mt-1.5`}>
                {accountType === 'us' ? 'Supports ACH, ACH same-day, and Wire payouts.' : 'Supports SEPA payouts.'}
              </p>
            ...
            <button onClick={submit} disabled={submitting || !canUseExternalAccounts} className="w-full py-3.5 rounded-2xl bg-[#C7FF00] text-black font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-60 mt-2">
