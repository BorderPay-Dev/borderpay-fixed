import {
  exactServiceCredential,
  providerLinkReason,
  restrictionReason,
} from "../supabase/functions/bridge-missing-customer-migration/policy.ts";
...
Deno.test("repair preserves local and provider restrictions", () => {
  for (
    ...
    for (
      ...
      eq(
        restrictionReason({ ...profile, [field]: state }, null, auth),
        "account_restricted",
      );
    }
  }
  eq(
    restrictionReason(profile, { bridge_kyb_status: "rejected" }, auth),
    "account_restricted",
  );
  eq(
    restrictionReason(
      { ...profile, account_frozen_reason: "fraud recall" },
      null,
      auth,
    ),
    "account_restricted",
  ...
  eq(
    restrictionReason(profile, null, { ...auth, banned_until: "2099-01-01" }),
    "auth_account_restricted",
  );
});
Deno.test("repair rejects operator, demo, unrelated-provider, and changed-email profiles", () => {
  eq(
    restrictionReason({ ...profile, is_demo: true }, null, auth),
    "operator_or_demo_account",
  );
  eq(
    restrictionReason({ ...profile, payment_provider: "other" }, null, auth),
    "different_provider",
  );
  eq(
    restrictionReason(profile, null, { email: "other@example.invalid" }),
    "auth_email_mismatch",
  );
...
Deno.test("existing provider IDs require exact identity match and unrestricted status", () => {
  const raw = {
    email: profile.email,
    type: "business",
    business_name: "Synthetic Ltd",
    status: "not_started",
    metadata: { borderpay_user_id: profile.id },
  };
  const business = { company_name: "Synthetic Ltd" };
  eq(providerLinkReason(raw, profile, business), null);
  eq(
    providerLinkReason({ ...raw, status: "paused" }, profile, business),
    "provider_customer_restricted",
  );
  eq(
    providerLinkReason(
      { ...raw, email: "someone@example.invalid" },
      profile,
      business,
    ),
  ...
  eq(
    providerLinkReason(
      { ...raw, business_name: "Other Ltd" },
      profile,
      business,
    ),
    "provider_company_name_mismatch",
  );
  eq(
    providerLinkReason(
      { ...raw, metadata: { borderpay_user_id: "other" } },
      profile,
      business,
    ),
    "provider_customer_owned_by_another_user",
  );
});
