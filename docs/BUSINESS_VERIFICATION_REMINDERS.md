...
The live customer GET response can omit all company-name fields. For reminders to an already-linked customer, require exact returned customer ID, unique ownership, matching confirmed email and business type; compare company name only when returned. New-customer repair/linking continues to require the business name. Do not infer a company name from first_name/last_name fields.
