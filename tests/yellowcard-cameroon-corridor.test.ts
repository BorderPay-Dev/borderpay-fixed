...
Deno.test("Cameroon fees use PDF pricing plus the fee-type-specific BorderPay markup", () => {
  ...
  if (!close(send?.provider_fee_percent, 1.02) || !close(send?.customer_fee_percent, 3.02) || !close(send?.provider_amount_local, 656) || !close(send?.borderpay_amount_local, 328) || !close(send?.customer_amount_local, 984)) {
