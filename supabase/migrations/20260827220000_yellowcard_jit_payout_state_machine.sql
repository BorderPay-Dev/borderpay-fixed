...
create table if not exists public.yellowcard_jit_payouts (
  ...
  settlement_asset text not null check (settlement_asset in ('USDC','USDT')),
  ...
  check ((settlement_asset = 'USDT' and settlement_network = 'TRON') or
         (settlement_asset = 'USDC' and settlement_network = 'BASE'))
);
...
revoke insert, update, delete, truncate on public.yellowcard_jit_payouts from anon, authenticated;
grant select on public.yellowcard_jit_payouts to authenticated;
grant select, insert, update on public.yellowcard_jit_payouts to service_role;
revoke insert, update, delete, truncate on public.yellowcard_jit_payout_events from anon, authenticated;
grant select on public.yellowcard_jit_payout_events to authenticated;
...
create policy yellowcard_jit_payout_events_owner_read on public.yellowcard_jit_payout_events
  for select to authenticated using (
    exists (
      ...
      where p.id = payout_id and p.user_id = (select auth.uid())
    )
  );
...
begin
  ...
  where p.bridge_wallet_id=p_bridge_wallet_id
    and p.state in ('PENDING_SWEEP','SEND_INTENT_CREATED','TREASURY_SWEEP_SENT','REFUND_PENDING');
...
begin
  ...
  with candidates as (
    select p.id
    from public.yellowcard_jit_payouts p
    where p.state in ('PENDING_SWEEP','SEND_INTENT_CREATED','TREASURY_SWEEP_SENT')
...
begin
  ...
  v_allowed :=
    ...
    (v_row.state='TREASURY_SWEEP_SENT' and p_to_state in ('YELLOW_CARD_CREDITED','COMPLETED','REFUND_PENDING','FAILED')) or
    (v_row.state='YELLOW_CARD_CREDITED' and p_to_state in ('DISPATCHED_TO_RAILS','COMPLETED','REFUND_PENDING','FAILED')) or
    (v_row.state='DISPATCHED_TO_RAILS' and p_to_state in ('COMPLETED','REFUND_PENDING','FAILED')) or
    (v_row.state='REFUND_PENDING' and p_to_state='FAILED') or
