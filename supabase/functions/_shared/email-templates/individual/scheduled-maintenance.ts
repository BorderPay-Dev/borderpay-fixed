...
export interface IndividualScheduledMaintenanceProps {
  full_name?: string;
...
export function render(p: IndividualScheduledMaintenanceProps): RenderedEmail {
  ...
  const introText = `Hello ${name}, we have a short maintenance window coming up.`;
  const bodyText =
    `BorderPay will be temporarily unavailable on ${maintenanceDate}, starting at ${maintenanceTime}. ` +
    `The maintenance is expected to last ${maintenanceDuration}. During this time, you may not be able to sign in, view account details, or complete payment actions.`;
  const safetyText = "Your funds and account information remain safe. No action is required from you.";
