...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: securityData } = await supabase
      .from("user_security")
      .select("pin_set, two_factor_enabled, two_factor_secret_encrypted")
      .eq("user_id", user.id)
      .maybeSingle();
    ...
    if (accountType === "business") {
      const { data: biz } = await supabase
        ...
        .select("bridge_kyb_status, bridge_customer_id, country")
        .eq("user_id", user.id)
        .maybeSingle();
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        user: {
          ...
          country:             businessCountry || profile?.country || userData?.country || null,
          ...
          bridge_customer_id:  businessBridgeCustomerId || profile?.bridge_customer_id || null,
          ...
          // Do not expose an enabled factor unless enrollment has a durable
          // server-side secret. This also repairs stale native-client caches.
          two_factor_enabled:  securityData?.two_factor_enabled === true && Boolean(securityData?.two_factor_secret_encrypted),
