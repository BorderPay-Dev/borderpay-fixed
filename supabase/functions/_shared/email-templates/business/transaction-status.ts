...
export function render(p: TransactionStatusProps): RenderedEmail {
  ...
  const displayIntro = isApprovedReceipt ? "Expected same day" : `${company}: ${c.intro}`;
  const subject = isApprovedReceipt ? `${company}: payment submitted (${outgoing})` : `${company}: ${c.subject.toLowerCase()} (${outgoing})`;
  ...
  const feeText = serviceChargeAmount > 0
    ? `A ${fmtMoney(serviceChargeAmount, sourceCurrency)} service charge was applied.`
  ...
  const receiptSummary = hasReceipt
    ? `We received ${fmtMoney(sourceAmount, sourceCurrency)}${railText} for ${company}. ${feeText} The available amount was converted and ${outgoing} was sent${destinationText}.`
    : "";
  const receiptRows = hasReceipt
    ? `
      ...
      <tr><td style="padding:8px 0;color:${BORDERPAY_BRAND.textMuted};font-size:13px;">Incoming funds</td>
          <td style="padding:8px 0;color:${BORDERPAY_BRAND.text};font-size:13px;font-family:'DM Mono',monospace;text-align:right;">${escapeHtml(fmtMoney(sourceAmount, sourceCurrency))}</td></tr>
      ...
      <tr><td style="padding:8px 0;color:${BORDERPAY_BRAND.textMuted};font-size:13px;">Outgoing funds</td>
          <td style="padding:8px 0;color:${BORDERPAY_BRAND.text};font-size:13px;font-weight:700;font-family:'DM Mono',monospace;text-align:right;">${escapeHtml(outgoing)}</td></tr>
