...
const [businessSubscription, businessOwner, businessEmailToken] = await Promise.all([
  serviceRest(`subscriptions?user_id=eq.${businessId}&select=user_id,account_type,status`),
  serviceRest(`business_team_members?business_user_id=eq.${businessId}&member_user_id=eq.${businessId}&select=business_user_id,member_user_id,role,status`),
  serviceRest(`email_verification_tokens?user_id=eq.${businessId}&select=user_id,purpose`),
]);
record(
  "Business owner and verification records are created without premature billing",
  businessSubscription.json?.length === 0 &&
    businessOwner.json?.[0]?.role === "owner" &&
    businessOwner.json?.[0]?.status === "active" &&
    businessEmailToken.json?.length === 1,
  `subscription=${businessSubscription.json?.length}, owner=${businessOwner.json?.length}, email_token=${businessEmailToken.json?.length}`,
);
