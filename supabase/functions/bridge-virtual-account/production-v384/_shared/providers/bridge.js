...
import { buildBridgeTransferBody } from "./bridge-transfer-payload.js";
...
export function liquidationAddressDeveloperFeeFields(chain, developerFeePercent) {
    const feePercent = String(developerFeePercent || "").trim();
    if (!feePercent || String(chain).toLowerCase() === "tron")
        return {};
    return {
...
export class BridgeProvider {
    ...
    async createCustomer(input) {
        ...
        if (input.account_type === "individual") {
            ...
            if (input.phone_e164)
                body.phone = input.phone_e164;
        }
        else {
            body.business_legal_name = input.company_name;
            if (input.registration_number)
                body.business_registration_number = input.registration_number;
        }
        body.metadata = {
    ...
    async createKycLink(input) {
        ...
        if (input.customer_id) {
            ...
            if (input.account_type === "individual") {
                if (input.full_name)
                    body.full_name = input.full_name;
            }
            else {
    ...
    async findCustomerByEmail(email) {
        ...
        for (let page = 0; page < 20; page += 1) {
            ...
            const next = pagination?.next ?? pagination?.next_cursor ?? pagination?.after ?? (r.data?.has_more && rows.length ? rows[rows.length - 1]?.id : undefined);
            if (!next || !rows.length)
                break;
            cursor = String(next);
        }
    ...
    /** Fetch canonical customer profile fields from Bridge. */ async getCustomerProfile(customerId) {
        const r = await bridgeFetch({
            method: "GET",
            path: `/v0/customers/${encodeURIComponent(customerId)}`
        });
    ...
    /** List the customer's custodial stablecoin wallets. */ async listWallets(customerId) {
    ...
    /** Get per-wallet balance rows from Bridge. */ async getWalletBalances(customerId, walletId) {
        const r = await bridgeFetch({
            method: "GET",
    ...
    /** List the customer's USD/EUR/GBP virtual accounts. */ async listVirtualAccounts(customerId) {
        const rows = await this.fetchBridgeListPaginated({
    ...
    /** Deactivate a VA so it cannot accept new incoming transactions. */ async deactivateVirtualAccount(customerId, virtualAccountId) {
        const r = await bridgeFetch({
            method: "POST",
    ...
    /** Reactivate a provider VA instead of attempting to create a duplicate. */ async reactivateVirtualAccount(customerId, virtualAccountId) {
        const r = await bridgeFetch({
            method: "POST",
    ...
    async fetchBridgeListPaginated(params) {
        ...
        while (page < maxPages) {
            ...
            if (rows.length === 0)
                break;
            if (rows.length < pageSize && !hasMore && !nextCursor)
                break;
            if (nextCursor && seenCursors.has(nextCursor))
                break;
            if (!nextCursor && firstId && previousFirstId && firstId === previousFirstId)
                break;
    ...
    async createLiquidationAddress(input) {
        const body = {
