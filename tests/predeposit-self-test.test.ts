...
import {evaluateInvoice,assessedDigest,applyInvoiceReviewMode} from "../supabase/functions/_shared/predeposit-policy.ts";
Deno.test("synthetic worker probe uses only credential RPC and never writes production records",async()=>{
 ...
 const report=await runPredepositSelfTest(db,async(fake,row)=>{
  ...
  return {pending:false,context,assessment:{...applyInvoiceReviewMode(evaluateInvoice(row.payload,context,ai),"automatic"),payload_sha256:row.payload_sha256,assessed_sha256:ai.payload_sha256,review_context:context,config_sha256:row.review_context.config_sha256,deterministic_status:"ready_for_ai"}};
 });
