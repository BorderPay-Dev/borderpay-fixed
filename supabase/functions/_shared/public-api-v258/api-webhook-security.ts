...
async function importEncryptionKey(encodedKey: string): Promise<CryptoKey> {
  ...
  if (bytes.byteLength !== 32) {
    throw new ApiWebhookSecurityError("Webhook encryption key must contain exactly 32 bytes");
  }
...
function secretAad(endpointId: string, version: number): Uint8Array {
  if (!endpointId || !Number.isInteger(version) || version < 1) {
    throw new ApiWebhookSecurityError("Endpoint identity and secret version are required");
  }
...
export function readApiWebhookEncryptionKey(): string {
  ...
  if (!value) {
    throw new ApiWebhookSecurityError("API webhook encryption is not configured");
  }
  return value;
}

export async function encryptApiWebhookSecret(
  secret: string,
  endpointId: string,
  version: number,
  encodedKey = readApiWebhookEncryptionKey(),
  ...
  const ciphertext = await crypto.subtle.encrypt(
    {
      ...
      additionalData: asArrayBuffer(secretAad(endpointId, version)),
    },
    key,
...
export async function decryptApiWebhookSecret(
  ciphertext: string,
  nonce: string,
  endpointId: string,
  version: number,
  ...
  try {
    ...
    const plaintext = await crypto.subtle.decrypt(
      {
        name: "AES-GCM",
        iv: asArrayBuffer(base64ToBytes(nonce)),
        additionalData: asArrayBuffer(secretAad(endpointId, version)),
      },
      key,
...
export async function signApiWebhookPayload(
  secret: string,
  timestamp: string,
  body: string,
): Promise<string> {
  if (!secret || !/^\d{10,}$/.test(timestamp)) {
    throw new ApiWebhookSecurityError("Webhook signing inputs are invalid");
  }
...
export function validateApiWebhookEndpointUrl(value: string): string {
  ...
  try {
    ...
    throw new ApiWebhookSecurityError("Webhook endpoint must be a valid HTTPS URL");
  }
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) {
    throw new ApiWebhookSecurityError("Webhook endpoint must use HTTPS without embedded credentials");
  }
