...
Deno.serve(async (req) => {
  ...
  try {
    if (action === "list_tenants") {
      const [tenantsResult, approvalsResult, partnerResult] = await Promise.all([
        ...
        supa.from("partner_organizations").select("id,status,approved_tenant_id,partner_applications(id,status,version,requested_products,created_at)"),
      ]);
      if (partnerResult.error) throw new Error("Partner KYB status could not be loaded");
      ...
      const linkedPartner = (tenantId: string) => {
        const matches = (partnerResult.data ?? []).filter((org: any) => org.approved_tenant_id === tenantId);
        if (matches.length !== 1) return null;
        const org = matches[0];
        const application = [...(org.partner_applications || [])].sort((a: any,b: any) => b.version-a.version || String(b.created_at).localeCompare(String(a.created_at)))[0];
        return { organization_id: org.id, status: org.status, application_id: application?.id, application_status: application?.status, requested_products: application?.requested_products || [] };
      };
      const data = (tenantsResult.data ?? []).map((tenant: any) => ({
        ...tenant,
        partner_approval: approvals.get(String(tenant.id)) ?? null,
        partner_kyb: linkedPartner(String(tenant.id)),
      }));
    ...
    if (action === "create_api_key") {
      ...
      let approval = await loadPartnerApproval(supa, tenantId);
      if (!approval && tenant.default_mode === "sandbox") {
        const reconciled = await supa.rpc("reconcile_partner_sandbox_approval", { p_tenant_id: tenantId, p_actor: actorId });
        if (reconciled.error) return json({ success: false, code: "partner_approval_sync_required", error: reconciled.error.message }, 409);
        approval = reconciled.data;
      }
