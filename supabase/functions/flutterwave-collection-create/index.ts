...
Deno.serve(async (req) => {
  ...
  if (!caps.configured || !caps.receive_enabled) {
    await recordAfricanRailsOperatorAlert(supa, {
      ...
      code: !caps.configured ? "flutterwave_not_configured" : "flutterwave_receive_disabled",
      message: "Controlled Flutterwave receive test is blocked by runtime configuration.",
    });
    return json({
      success: false,
