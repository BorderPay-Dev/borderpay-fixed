import { BORDERPAY_BRAND, htmlLayout, RenderedEmail, textLayout } from "../layout.ts";

export interface BusinessAfricanRailsLiveProps {
  company_name?: string;
  action_url?: string;
}

export function render(p: BusinessAfricanRailsLiveProps): RenderedEmail {
  ...
  const introText = `${company} can now access supported African bank and mobile money rails through BorderPay.`;
  const body = `
    <p style="margin:0 0 14px;color:#111111;font-size:14px;line-height:1.65;text-align:left;">
      Use BorderPay to send business payments through the bank and mobile money methods currently available for each supported African market.
    </p>
