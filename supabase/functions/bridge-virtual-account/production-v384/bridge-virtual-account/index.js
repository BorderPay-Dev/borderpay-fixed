...
import { bridgeProvider, BridgeProviderError } from "../_shared/providers/bridge.js";
import { isBridgeBlocked, bridgeCountryBlockResponse, logControlledBridgeTraffic, isBridgeVirtualAccountCurrencyAvailable, } from "../_shared/providers/bridge-country-policy.js";
import { requireMinimumWalletBalance } from "../_shared/funding-gate.js";
import { loadAndAssertBridgeIdentityInvariant } from "../_shared/bridge-identity-invariant.js";
import { loadVirtualAccountDestinationConfig, } from "../_shared/providers/virtual-account-config.js";
import { borderPayDirectVaDeveloperFeePercent } from "../_shared/fees/schedule.js";
import { loadBridgeEeaWalletSecurityEnrollment, walletSecurityEnrollmentResponse, } from "../_shared/wallet-security-enrollment.js";
import { bridgeEeaPilotAccessRequired, bridgeEeaPilotAccessAllowed, bridgeEeaScaEnforcementEnabled, } from "../_shared/bridge-sca-scope.js";
const CORS = {
...
const RAIL_BY_CCY = { USD: "ach_push", EUR: "sepa", GBP: "faster_payments" };
const DEFAULT_ZERO_FEE_EMAILS = new Set(["adhiamboadhiambo22@gmail.com"]);
function normalizeLocalVaStatus(value) {
    const status = String(value || "").trim().toLowerCase();
    if (["deactivated", "inactive"].includes(status))
        return "deactivated";
    if (["closed", "deleted", "disabled"].includes(status))
        return "closed";
    if (status === "suspended" || status === "paused")
        return "suspended";
    return "active";
}
function normalizeLocalVaRail(value) {
    const rail = String(value || "").trim().toLowerCase();
    if (!rail)
        return null;
    if (rail === "ach")
        return "ach_push";
    if (["ach_push", "ach_pull", "wire", "sepa", "faster_payments"].includes(rail))
        return rail;
    return null;
}
...
function bridgeValidationDetails(rawText) {
    if (!rawText)
        return null;
    try {
...
function bridgeValidationText(rawText) {
...
function isZeroFeeVirtualAccountUser(email) {
    const normalized = String(email || "").trim().toLowerCase();
    if (!normalized)
        return false;
    if (DEFAULT_ZERO_FEE_EMAILS.has(normalized))
        return true;
...
async function notifyOpsIncident(input) {
    if (!SUPABASE_URL || !SEND_EMAIL_TOKEN)
        return;
...
export const legacyVirtualAccountHandler = async (req) => {
    ...
    if (action === "capabilities") {
        ...
        const supported_currencies = ["USD", "EUR", "GBP"].filter((c) => isBridgeVirtualAccountCurrencyAvailable(productCountry, c));
        const { data: pendingRows } = await supa
            .from("pending_va_requests")
            .select("currency, bridge_error_code, status")
            .eq("user_id", user.id)
            .eq("status", "pending");
        const providerPendingCurrencies = new Set((pendingRows || [])
            .filter((r) => ["provider_not_allowed", "endorsement_not_granted"].includes(String(r.bridge_error_code || "").toLowerCase()))
            .map((r) => String(r.currency || "").toUpperCase())
            .filter((c) => c === "USD" || c === "EUR" || c === "GBP"));
        const configured_currencies = [];
        const setup_pending_currencies = [];
    ...
    // Legacy minimum-balance gate retained as a compatibility no-op.
    {
        ...
        if (!__fund.allowed)
            return json(__fund.body, __fund.status);
    }
    ...
    try {
        ...
        let bridgeVa = sameCurrencyBridgeAccounts.find((v) => ["active", "activated"].includes(String(v.status || "active").toLowerCase())) ?? sameCurrencyBridgeAccounts.find((v) => String(v.status || "").toLowerCase() === "deactivated") ?? sameCurrencyBridgeAccounts[0];
        ...
        if (bridgeVa?.virtual_account_id) {
            ...
            const persistedFee = normalizeDeveloperFeePercent(bridgeVa.developer_fee_percent) ??
                normalizeDeveloperFeePercent(details?.developer_fee_percent);
    ...
    if (scaEnrollment.required) {
        const { data: baseWallet, error: baseWalletError } = await supa
            ...
            .maybeSingle();
        if (baseWalletError) {
            console.error("bridge_eea_va_wallet_lookup_failed", {
                user_id: user.id,
                currency,
                error: baseWalletError.message,
            });
            return json({
                success: false,
                code: "wallet_status_unavailable",
                error: "Wallet status is temporarily unavailable.",
            }, 503);
        }
    ...
    let destination;
    let developerFeePercent;
    let idempotencyKey;
    try {
        // Settlement asset follows the same authoritative Bridge jurisdiction as
        // SCA. For businesses this is the legal country of incorporation, never
        // the operating/profile country used for product availability.
        const settlementJurisdiction = scaEnrollment.country || productCountry;
        destination = await loadVirtualAccountDestinationConfig(supa, currency, {
            userId: user.id,
            bridgeCustomerId: profile.bridge_customer_id,
            country: settlementJurisdiction,
        });
    ...
    try {
        ...
        const srcDep = (raw.source_deposit_instructions && typeof raw.source_deposit_instructions === "object")
            ? raw.source_deposit_instructions
            : {};
        const persistedFee = normalizeDeveloperFeePercent(raw?.developer_fee_percent) ??
        ...
        const isGrantPending = providerCode.includes("endorsement") ||
        ...
        if (e instanceof BridgeProviderError) {
            ...
            if (currency === "USD" && providerCode === "invalid_parameters") {
                const testingVaLimitExceeded = /exceeds limit of testing virtual account numbers/i.test(bridgeValidationText(e.raw_text));
                try {
                    await savePendingVaRequest({
                        userId: user.id,
                        bridgeCustomerId: profile.bridge_customer_id,
