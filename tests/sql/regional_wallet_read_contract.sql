...
insert into public.bridge_wallets(user_id,currency,chain) select id,'EURC','base' from public.user_profiles;
insert into public.bridge_balance_ledger(user_id,currency) select id,'EURC' from public.user_profiles;
insert into public.wallets(user_id,currency,balance) select id,'EURC',10 from public.user_profiles;
...
begin
 for n in 1..35 loop
  ...
  if public.can_read_borderpay_eurc(uid) is distinct from (n <= 30) then raise exception 'Wrong EURC eligibility %', n; end if;
  select count(*) into count_rows from public.bridge_wallets where currency='EURC';
  if count_rows <> (case when n <= 30 then 1 else 0 end) then raise exception 'Wrong EURC wallet visibility %',n; end if;
  select count(*) into count_rows from public.bridge_balance_ledger where currency='EURC';
  if count_rows <> (case when n <= 30 then 1 else 0 end) then raise exception 'Wrong EURC ledger visibility %',n; end if;
  select count(*) into count_rows from public.wallets where currency='EURC';
  if count_rows <> (case when n <= 30 then 1 else 0 end) then raise exception 'Wrong EURC legacy visibility %',n; end if;
...
rollback;
select 'Regional USDT/EURC owner-read contract: PASS' as result;
