...
checks = {
    ...
    "receiving rails support US EU and GB": all(token in UI for token in ("'US'", "'EU'", "'GB'", "'SEPA'", "Faster Payments", "ACH / Wire")),
    "receiving account list is provider-driven": ".map((account)" in UI and "ReceiveView" in UI,
    "transactions contain only master account provider activity": "BridgeTransferLedger" in UI and "Customer activity" not in UI and "customer_transactions" not in UI,
