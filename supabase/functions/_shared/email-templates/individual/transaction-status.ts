import { htmlLayout, textLayout, firstName, escapeHtml, fmtMoney, fmtReceiptMoney, BORDERPAY_BRAND, RenderedEmail } from "../layout.ts";
...
export function render(p: TransactionStatusProps): RenderedEmail {
  ...
  const displayIntro = isMoneyInConversion
    ? `Hi ${firstName(p.full_name)}, your incoming payment was converted and added to your BorderPay wallet.`
    : isApprovedReceipt ? "Expected same day" : `Hi ${firstName(p.full_name)}, ${c.intro}`;
  const subject = isMoneyInConversion ? `Funds received: ${outgoing}` : isApprovedReceipt ? `Your payment has been submitted: ${outgoing}` : `${c.subject}: ${outgoing}`;
  ...
  const receiptSummary = hasReceipt
    ? `We received ${isMoneyInConversion ? fmtReceiptMoney(sourceAmount, sourceCurrency) : fmtMoney(sourceAmount, sourceCurrency)}${railText}. ${feeText} ${isMoneyInConversion ? `${walletAmount} was added to your wallet.` : `The available amount was converted and ${outgoing} was sent${destinationText}.`}`
    : "";
