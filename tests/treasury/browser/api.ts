...
const snapshot = {
 ...
 treasury_valuation:{currency:'USD',total:'140939.90',as_of:new Date().toISOString(),history:{complete:true,points:Array.from({length:7},(_,i)=>({at:new Date(Date.now()-(6-i)*86400000).toISOString(),usd:i===6?140939.9:100000+i*5000}))}},
...
export const treasuryAPI = {
 getSnapshot:async()=>{const w=window as any;w.reads=(w.reads||0)+1;await new Promise(r=>setTimeout(r,w.readDelay||30));return w.failRead ? {success:false,error:'Fixture refresh unavailable'}:{success:true,data:{...snapshot,activity_history_complete:!w.partialHistory,treasury_valuation:{...snapshot.treasury_valuation,history:{...snapshot.treasury_valuation.history,complete:!w.partialHistory}}}};},
