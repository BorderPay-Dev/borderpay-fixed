/** Normalize Bridge list pagination into a stable list for BorderPay clients. */
export function extractBridgeExternalAccounts(payload: unknown): Record<string, unknown>[] {
  ...
  if (isRecord(payload.external_accounts)) {
    if (Array.isArray(payload.external_accounts.data)) return payload.external_accounts.data.filter(isRecord);
    if (Array.isArray(payload.external_accounts.items)) return payload.external_accounts.items.filter(isRecord);
  }
  ...
  if (isRecord(payload.data)) {
    ...
    if (isRecord(payload.data.external_accounts)) {
      if (Array.isArray(payload.data.external_accounts.data)) return payload.data.external_accounts.data.filter(isRecord);
      if (Array.isArray(payload.data.external_accounts.items)) return payload.data.external_accounts.items.filter(isRecord);
    }
