...
/** EEA member states: EU-27 plus Iceland, Liechtenstein and Norway.
 *  The United Kingdom and Switzerland are deliberately excluded. */ export const BRIDGE_EEA_COUNTRIES = new Set([
    "AT",
    "BE",
    "BG",
    "HR",
...
export function normalizeBridgeCountryCode(countryCode) {
    if (!countryCode)
        return null;
...
export function isBridgeEeaCountry(countryCode) {
    const upper = normalizeBridgeCountryCode(countryCode);
    return !!upper && BRIDGE_EEA_COUNTRIES.has(upper);
}
export const isBridgeEuropeanUnionCountry = isBridgeEeaCountry;
const EEA_AUTOMATIC_WALLETS = [
    {
        symbol: "USDC",
        chain: "BASE"
    },
...
/** Wallet policy for verified customers. EEA-30 customers use USDC and EURC
 * on Base. Existing provider wallets are never deleted here. */ export function bridgeAutomaticWalletsForCountry(countryCode) {
    return isBridgeEeaCountry(countryCode) ? EEA_AUTOMATIC_WALLETS : NON_EU_AUTOMATIC_WALLETS;
}
/** Returns true if Bridge classifies the country as Prohibited (sanctions). */ export function isBridgeProhibited(countryCode) {
    const upper = normalizeBridgeCountryCode(countryCode);
    return !!upper && BRIDGE_PROHIBITED_COUNTRIES.has(upper);
}
/** Returns true if Bridge has marked the country as Unavailable
 *  (commercial/regulatory, not sanctions). */ export function isBridgeUnavailable(countryCode) {
    const upper = normalizeBridgeCountryCode(countryCode);
    return !!upper && BRIDGE_UNAVAILABLE_COUNTRIES.has(upper);
}
/** Returns true if Bridge classifies the country as Controlled / High Risk.
 *  Used by the observability logger; NOT a blocker per round-9 policy. */ export function isBridgeControlled(countryCode) {
    const upper = normalizeBridgeCountryCode(countryCode);
 ...
 *  observability. */ export function isBridgeBlocked(countryCode) {
...
export function bridgeVirtualAccountCurrenciesForCountry(countryCode) {
    if (!countryCode || isBridgeBlocked(countryCode))
        return [];
    const upper = normalizeBridgeCountryCode(countryCode);
    if (!upper)
        return [];
    const currencies = [];
    if (!BRIDGE_VA_NO_US_RAIL.has(upper))
        currencies.push("USD");
    if (!BRIDGE_VA_NO_SEPA_FPS_RAIL.has(upper))
        currencies.push("EUR", "GBP");
    return currencies;
}
export function isBridgeVirtualAccountCurrencyAvailable(countryCode, currency) {
    if (!currency)
        return false;
    return bridgeVirtualAccountCurrenciesForCountry(countryCode).includes(String(currency).toUpperCase());
}
export function isBridgeCustodialWalletSupported(countryCode) {
    if (!countryCode || isBridgeBlocked(countryCode))
        return false;
    const upper = normalizeBridgeCountryCode(countryCode);
    return !!upper && !BRIDGE_CUSTODIAL_WALLET_UNSUPPORTED_COUNTRIES.has(upper);
}
export function bridgeCountryTier(countryCode) {
    const upper = normalizeBridgeCountryCode(countryCode);
    if (!upper)
        return "supported";
    if (BRIDGE_PROHIBITED_COUNTRIES.has(upper))
        return "prohibited";
    if (BRIDGE_UNAVAILABLE_COUNTRIES.has(upper))
        return "unavailable";
    if (BRIDGE_CONTROLLED_COUNTRIES.has(upper))
        return "controlled";
    return "supported";
}
 ...
 *  fire for them). */ export function bridgeCountryBlockResponse(countryCode) {
 ...
 *
 *  Format is grep-friendly: `bridge_controlled_country fn=<…> country=<…> user_id=<…>`. */ export function logControlledBridgeTraffic(fn, countryCode, userId) {
    if (!countryCode)
        return;
...
export function isAfricanPayoutCountry(countryCode) {
