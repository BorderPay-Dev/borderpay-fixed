 ...
 * - Keep issuance and controls locked until program activation.
 */
...
export function CardsScreen({ onBack: _onBack }: CardsScreenProps) {
  ...
  const subtitle    = (t as any)?.('cards.subtitle') ?? 'Card issuance is not available yet';
    ...
    <div className={`min-h-screen ${tc.bg}`}>
      <div className="max-w-2xl mx-auto px-4 sm:px-5 pt-5 pb-10">
        ...
        <div className="mt-6 max-w-2xl">
          <h1 className={`text-2xl sm:text-3xl font-semibold ${tc.text} tracking-tight mb-2`}>
            Card issuance is not available yet
          </h1>
          <p className={`text-sm ${tc.textMuted} leading-relaxed`}>Explore the card interface while issuance remains locked.</p>
          {message && (
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <button
              ...
              aria-disabled
              onClick={() => {
                setMessage('Card creation is locked.');
              }}
            ...
            <button
              ...
              aria-disabled
              onClick={() => {
                setMessage('Spending controls are locked.');
              }}
              className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border ${tc.cardBorder} ${tc.text} ${tc.hoverBg}`}
            >
