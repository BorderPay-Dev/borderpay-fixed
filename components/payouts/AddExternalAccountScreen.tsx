 ...
 *
 * Bridge v1 scope:
 *   • US bank account (USD): ACH / ACH same-day / Wire chosen at transfer time.
 *   • IBAN (EUR): SEPA.
 */
  ...
  useEffect(() => {
    ...
    const loadCapabilities = async (force = false) => {
      ...
      try {
        ...
        // Keep seeded state on transient network failures.
      } finally {
    ...
    try {
      ...
      if (accountType === 'us') {
        if (!accountNumber.trim() || !routingNumber.trim()) {
          toast.error('Account number and routing number are required.');
          setSubmitting(false);
          return;
        }
        if (!street.trim() || !city.trim() || !postal.trim() || !country.trim()) {
          toast.error('A full billing address is required for US accounts.');
          setSubmitting(false);
          return;
        }
        ...
        if (!iban.trim() || !bic.trim() || !ibanCountry.trim()) {
          toast.error('IBAN, BIC/SWIFT, and IBAN country are required.');
          setSubmitting(false);
          return;
        }
      ...
      <main className="px-5 sm:px-6 pb-10 max-w-md mx-auto space-y-4">
        {capabilityLoading ? (
          ...
          <>
            ...
            <button onClick={onBack} className={`w-full py-2.5 text-xs ${tc.textMuted}`}>
              Cancel
            </button>
          </>
        )}
