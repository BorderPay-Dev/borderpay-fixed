/** ISO 3166-1 alpha-2/alpha-3 pairs; jurisdiction policy is defined separately. */
const pairs = `
...
OM OMN PA PAN PE PER PF PYF PG PNG PH PHL PK PAK PL POL PM SPM PN PCN PR PRI PS PSE PT PRT PW PLW PY PRY QA QAT
...
`.trim().split(/\s+/);
export const ISO3_TO_ISO2: Readonly<Record<string, string>> = Object.freeze(Object.fromEntries(
  Array.from({ length: pairs.length / 2 }, (_, index) => [pairs[index * 2 + 1], pairs[index * 2]]),
));
export const ISO2_COUNTRIES: ReadonlySet<string> = new Set(Object.values(ISO3_TO_ISO2));
