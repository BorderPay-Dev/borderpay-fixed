  ...
  return (
    <main className="fixed inset-0 z-[10000] flex min-h-[var(--app-height)] items-center justify-center overflow-y-auto bg-[#080A0D] px-5 py-8 text-white">
      <section className="w-full max-w-md rounded-3xl border border-white/10 bg-[#11151A] p-6 shadow-2xl sm:p-8">
        ...
        <div className="mb-4 inline-flex rounded-full border border-amber-300/20 bg-amber-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-amber-200">Account frozen</div>
        ...
        <p className="mt-5 text-xs leading-5 text-white/45">You do not need to submit another verification request. BorderPay will contact you if anything is required.</p>
        <button type="button" onClick={onSignOut} className="mt-7 h-12 w-full rounded-2xl border border-white/10 bg-white/[0.06] text-sm font-semibold text-white transition-colors hover:bg-white/10 active:scale-[0.99]">Sign out</button>
      </section>
    </main>
  );
}
