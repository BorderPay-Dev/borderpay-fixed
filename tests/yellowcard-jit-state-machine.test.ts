...
Deno.test("ordinary Bridge payouts subtract active Yellow Card reservations", async () => {
  ...
  assert(bridge.includes('["PENDING_SWEEP", "SEND_INTENT_CREATED", "TREASURY_SWEEP_SENT", "REFUND_PENDING"]'), "active reservation states are incomplete");
  assert(bridge.includes('select("customer_debit_amount_minor")'), "gross reservation is not queried");
...
Deno.test("production JIT Send is fail closed and contains no sandbox execution path", async () => {
  const payout = await Deno.readTextFile(new URL(
    "../supabase/functions/yellowcard-jit-payout/index.ts",
    import.meta.url,
  ));
  ...
  for (const source of [payout, worker]) {
    assert(source.includes('flag("YC_PRODUCTION_SEND_ENABLED")'), "production Send gate missing");
    assert(source.includes('flag("YC_JIT_PAYOUT_ENABLED")'), "JIT activation gate missing");
    assert(!source.toLowerCase().includes("sandbox"), "sandbox execution reference remains in JIT source");
  }
});
