...
as $$
  with base as (
    select
      ...
      nullif(lower(coalesce(up.bridge_kyc_link_id, '')), '') as bridge_kyc_link_id
    from public.user_profiles up
    where up.bridge_customer_id is not null
      and up.created_at <= (now() - make_interval(days => greatest(p_age_days, 1)))
      and lower(coalesce(up.bridge_kyc_status::text, 'not_started')) <> 'approved'
  ),
  biz as (
    select
      bp.user_id,
      lower(coalesce(bp.bridge_kyb_status::text, 'not_started')) as bridge_kyb_status,
      nullif(lower(coalesce(bp.bridge_kyb_link_id, '')), '') as bridge_kyb_link_id
  ...
  select
    ...
    b.bridge_kyc_link_id,
    bz.bridge_kyb_link_id,
    c.va_count,
    c.wallet_count,
    c.transfer_count,
    c.external_count
  ...
  where coalesce(c.va_count, 0) = 0
    ...
    and (
      ...
      or coalesce(bz.bridge_kyb_status, 'not_started') <> 'approved'
    )
