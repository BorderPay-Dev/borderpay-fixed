...
function isStaticTransferTemplate(obj) {
  ...
  return boolish(features.static_template) || boolish(obj.static_template) || !!firstNonEmptyString(paymentRoute.id, obj.payment_route_id) || transferType === "static_template" || transferType === "payment_route";
}
export function validateBridgePayloadContract(routeBucket, eventTypeRaw, payloadRaw) {
  const payload = asRecord(payloadRaw);
  const obj = eventObject(payload);
  const customerId = firstNonEmptyString(obj.customer_id, obj.customer?.id, payload.event_object_id, obj.id);
  ...
  const currency = firstNonEmptyString(obj.currency, obj.source?.currency);
  ...
  switch(routeBucket){
    case "bridge.kyc":
      if (!customerId) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_customer_id"
        };
      }
      if (!status && (eventType.startsWith("kyc_link.") || eventType.startsWith("kyb_link.") || eventType.startsWith("customer.kyc") || eventType.startsWith("customer.kyb"))) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_verification_status"
        };
      }
      return {
        valid: true,
        reason_code: "payload_contract_ok"
      };
    case "bridge.virtual_account":
      return virtualAccountId ? {
        ...
        valid: false,
        reason_code: "invalid_payload_contract_missing_virtual_account_id"
      };
    case "bridge.wallet":
      return walletId ? {
        valid: true,
        ...
        valid: false,
        reason_code: "invalid_payload_contract_missing_wallet_id"
      };
    case "bridge.external_account":
      return externalAccountId ? {
        ...
        valid: false,
        reason_code: "invalid_payload_contract_missing_external_account_id"
      };
    case "bridge.liquidation_address":
      if (!liquidationAddressId) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_liquidation_address_id"
        };
      }
      if (eventType.includes("drain") && !status) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_drain_state"
        };
      }
      if (!eventType.includes("drain")) {
        return {
          valid: true,
          reason_code: "payload_contract_liquidation_address_log_only",
    ...
    case "bridge.transfer":
      if (!transferId) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_transfer_id"
        };
      }
      if (!status && eventType.startsWith("transfer.")) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_transfer_state"
        };
      }
      if ((eventType.endsWith(".created") || eventType.endsWith(".updated")) && (amount === undefined || amount === null || !currency)) {
        if (isStaticTransferTemplate(obj)) {
          return {
            valid: true,
            reason_code: "payload_contract_static_template_log_only",
            routing_target: "log_only"
          };
        }
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_transfer_amount_or_currency"
        };
      }
      return {
        valid: true,
    ...
    case "bridge.customer":
      ...
      if (eventType.startsWith("customer.") && !status) {
        return {
          valid: false,
          reason_code: "invalid_payload_contract_missing_customer_status"
        };
      }
      return {
        valid: true,
    ...
    default:
      return {
        valid: true,
        reason_code: "payload_contract_unknown_bucket"
      };
  }
}
