...
checks = {
    ...
    "gateway records customers": bool(re.search(r"register\(db, ctx, session, [\"']customer[\"']", customer_runtime)),
    "gateway records wallets": bool(re.search(r"register\(db, ctx, session, [\"']wallet[\"']", customer_runtime)),
    "gateway records virtual accounts": bool(re.search(r"session,\s*[\"']virtual_account[\"']", customer_runtime)),
    "gateway records payments": bool(re.search(r"session,\s*[\"']transfer[\"'],\s*transferId", customer_runtime)) and "handleCustomerApi" in gateway,
    ...
    "manual formation and identity evidence remains mandatory": all(x in portal for x in ["Certificate of incorporation", "Articles of association", "Director identity document"]),
    "supplemental compliance evidence is optional": "commonDocuments" not in portal and 'missing.push("Operating licence")' not in portal and 'missing.push("NDA")' not in portal,
