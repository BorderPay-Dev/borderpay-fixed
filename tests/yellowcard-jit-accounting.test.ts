...
import { parseYellowCardDirectSettlementSendInstruction } from "../supabase/functions/_shared/providers/yellowcard-payload.ts";
...
Deno.test("Yellow Card funding instruction must preserve the confirmed local payout", () => {
  const expected = {
  ...
  const instruction = parseYellowCardDirectSettlementSendInstruction(response, expected);
  if (instruction.convertedAmount !== 1000) throw new Error("local amount was not retained");
  let rejected = false;
  try {
    parseYellowCardDirectSettlementSendInstruction({ ...response, convertedAmount: 999 }, expected);
  } catch (error) {
    rejected = (error as Error).message === "yellow_card_destination_amount_mismatch";
  }
  if (!rejected) throw new Error("mismatched local payout was accepted");
});
